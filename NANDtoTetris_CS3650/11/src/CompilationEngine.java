 ...
 * It reads its input from a JackTokenizer and writes its output into a
 * VMWriter.
 * It is organized as a series of compilexxx ( ) routines, where xxx is a
 * syntactic element of the Jack language.
 * The contract between these routines is that each compilexxx ( ) routine
 * should read the syntactic construct xxx from the input,
 * advance ( ) the tokenizer exactly beyond xxx, and emit to the output VM code
 * effecting the semantics of xxx.
 * Thus compilexxx ( ) may only be called if indeed xxx is the next syntactic
 * element of the input.
 * If xxx is a part of an expression and thus has a value, the emitted code
...
public class CompilationEngine {
    ...
    private void compileClassVarDec() {
        ...
        // first determine whether there is a classVarDec, nextToken is } or start
        // subroutineDec
        tokenizer.advance();
    ...
    /**
     * write function declaration, load pointer when keyword is METHOD or
     * CONSTRUCTOR
     */
     ...
     * some of the alternative parsing rules.
    ...
    private void compileTerm() {
        ...
        if (tokenizer.tokenType() == JackTokenizer.TYPE.IDENTIFIER) {
            ...
            // integerConstant|stringConstant|keywordConstant|'(' expression ')'|unaryOp
            // term
    ...
    private void error(String val) {
        throw new IllegalStateException(
                "Expected token missing : " + val + " Current token:" + tokenizer.getCurrentToken());
    }
