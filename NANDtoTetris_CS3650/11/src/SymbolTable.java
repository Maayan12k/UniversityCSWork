 ...
 * Recall that each symbol has a scope from which it is visible in the source
 * code.
 * The symbol table implements this abstraction by giving each symbol a running
 * number (index) within the scope.
 * The index starts at 0, increments by 1 each time an identifier is added to
 * the table, and resets to 0 when starting a new scope.
 ...
 * table may be assumed to be a subroutine name or a class name.
 * Since the Jack language syntax rules suffice for distinguishing between these
 * two possibilities, and since no “linking” needs to be done by the compiler,
 ...
 * identifier properties needed for compilation:
 * type, kind, and running index. The symbol table for Jack programs has two
